...
plot_portfolio_returns_by_volatility <- function(ALSI, Zar, compute_returns, volatility_levels = c("Low Volatility", "Normal Volatility", "High Volatility"), J203_column = "J203", J403_column = "J403") {
    ...
    # Filter Zar data based on ALSI date and categorize currency
    Zar <- Zar %>% filter(as.Date(date) >= dplyr::first(ALSI$date)) %>% categorize_currency(value)
    ...
    # Merge portfolio returns with volatility levels from Zar
    ...
    # Create ridgeline plot for the portfolio returns by volatility level
