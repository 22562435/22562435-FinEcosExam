...
# Winsorize function
winsorize <- function(x, lower = 0.0, upper = 0.95) {
...
ridgeline_data <- ALSI %>%
  ...
  select(date, J203, J403, volatility) %>%
  mutate(
    J203 = winsorize(J203),
    J403 = winsorize(J403)
  ) %>%
  pivot_longer(cols = c(J203, J403), names_to = "Return_Type", values_to = "Return") %>%
  mutate(volatility = factor(volatility, levels = c("Low Volatility","Normal Volatility", "High Volatility")))

# Ridgeline plot
ggplot(ridgeline_data, aes(x = Return, y = volatility, fill = Return_Type)) +
  geom_density_ridges(alpha = 0.7, scale = 1) +
  scale_fill_manual(values = c("J203" = "blue", "J403" = "green"), labels = c("ALSI Returns", "SWIX Returns")) +
  labs(
    title = "Distribution of Winsorized ALSI and SWIX Returns by Volatility Level",
    x = "Return",
    y = "Volatility Level",
    fill = "Return Type"
  ) +
...
If this general approach aligns with your objective, let me know, and I’ll help craft the R code for the individual steps!
...
ALSI %>% filter(!is.na(Index_Name)) %>% plot_mean_returns_by_cap_size()
